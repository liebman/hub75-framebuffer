// Run with: cargo bench --bench render_text_latched
...
fn render_text_benchmark(c: &mut Criterion) {
    let mut group = c.benchmark_group("render_text_latched");
    ...
    for (case, text) in TEST_STRINGS {
        ...
        // Baseline
        group.bench_with_input(BenchmarkId::new("baseline", case), text, |b, &text| {
        ...
        // Optimised
        group.bench_with_input(BenchmarkId::new("optimised", case), text, |b, &text| {
            let origin = Point::new(0, 0);
            b.iter(|| {
                let mut fb = TestFrameBuffer::new();
                fb.clear();
